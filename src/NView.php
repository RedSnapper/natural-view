...
namespace RedSnapper\NaturalView;
...
class NView
{
    ...
    public function __construct($value, LoggerInterface $log = null)
    {
        ...
        match (gettype($value)) {
            'string' => $this->con_string($value),
            'object' => $this->con_object($value),
            ...
            default => throw new \InvalidArgumentException("Must be a resource or string")
        };
    }
     ...
     * 'show'
     */
    public function show(bool $wholeDocument = false)
    {
        $retval = "";
        if (!is_null($this->doc) && !is_null($this->xp)) {
            $this->tidyView();

            $retval = $this->doc->saveXML();
     ...
     * 'as_fragment'
     */
    private static function as_fragment($docstr)
    {
        $s1 = '/<\?xml[^?]+\?>/';
        $s2 = '/<!DOCTYPE \w+>/';
    ...
    public function strToNode($value = null)
    {
        ...
        return $this->doc->importNode($fnode, true);
    ...
    public function set($xpath, $value = null, $ref = null, $show_err_on_missing = true)
    {
        ...
        if (!is_null($this->doc) && !is_null($this->xp)) {
            ...
            switch (gettype($value)) {
                ...
                case "object" :
                    { //probably a node.
                        if (gettype($value) != "object" || is_subclass_of($value,
                                'DOMNode') || $value instanceof DOMNodeList || $value instanceof NView) {
                            if (is_null($ref)) {
                                $entries = $this->xp->query($xpath);
                            } else {
                                $entries = $this->xp->query($xpath, $ref);
                                        ...
                                        if ($gap == self::GAP_NATTR && $entry->nodeType == XML_ELEMENT_NODE && isset($aname)) {
                                            ...
                                            if (($entry->nodeType == XML_ATTRIBUTE_NODE) && (gettype($value) != "object")) {
                                                ...
                                                if ($value instanceof DOMNodeList) {
                                                            ...
                                                            case self::GAP_FOLLOWING:
                                                                {
                                                                    if (is_null($entry->nextSibling)) {
                                                                        ...
                                                                        $entry->parentNode->insertBefore($node,
                                                                            $entry->nextSibling);
                                                                    }
                                                                }
                                                                break;
                                                    ...
                                                    if (!is_a($node,
                                                            DOMDocumentFragment::class) || (!is_null($node->firstChild))) {
                                                        switch ($gap) {
                                                            case self::GAP_NONE:
                                                                {
                                                                    $entry->parentNode->replaceChild($node, $entry);
...
//                $value = str_replace($wss, "", $value); //str_replace should be mb safe.
     ...
     */
    private function doMsg($errno, $errstr = '', $errfile = '', $errline = 0)
    {
        if (!is_null($this->log)) {
    ...
    private function noMsg($errno, $errstr = '', $errfile = '', $errline = 0)
    {
